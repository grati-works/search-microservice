...
app.post('/user/:organization_id/:id', async (request: Request, response: Response) => {
...
app.get('/search/:organization_id', async (request: Request, response: Response) => {
  const { q } = request.query;
  const { organization_id } = request.params;

  const results = await sonicChannelSearch.query('users', organization_id, q, {
    lang: 'por',
  });
...
app.get('/suggest/:organization_id', async (request: Request, response: Response) => {
  const { q } = request.query;
  const { organization_id } = request.params;

  const results = await sonicChannelSearch.suggest('users', organization_id, q, {
    limit: 5,
  });
